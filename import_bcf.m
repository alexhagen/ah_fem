function [bcf] = import_bcf(filename,mesh)
    ...
    % find each set name
    start_ind_nset = regexp(assm,...
        '(\*Nset,\snset\=[A-Za-z1-9\-]*)');
    start_ind_elset = regexp(assm,...
        '(\*Elset,\selset\=[A-Za-z1-9\-]*)');
    % for each set name
    for i = 1:numel(start_ind_nset)
        %find the set name
        set = sscanf(assm(start_ind_nset(i):end),...
            '*Nset, nset=%s,');
        sets(i).name = set(1:end-1);
        %find the next newline
        next = regexp(assm(start_ind_nset(i):end), '[\f\n\r]');
        %find the nodes corresponding to this
        s = assm(start_ind_nset(i)+next(1):end);
        last = regexp(assm(start_ind_nset(i)+next(1):end),'(\*)');
        s = regexprep(assm(start_ind_nset(i)+next(1):...
            start_ind_nset(i)+next(1)+last(1)-2),'[\f\n\r]',' ');
        s = regexprep(s,',',' ');
        sets(i).nodes = sscanf(s,'%f ');
        %find the elements
        next = regexp(assm(start_ind_elset(i):end), '[\f\n\r]');
        %find the nodes corresponding to this
        s = assm(start_ind_elset(i)+next(1):end);
        last = regexp(assm(start_ind_elset(i)+next(1):end),'(\*)');
        s = regexprep(assm(start_ind_elset(i)+next(1):...
            start_ind_elset(i)+next(1)+last(1)-2),'[\f\n\r]',' ');
        s = regexprep(s,',',' ');
        sets(i).els = sscanf(s,'%f ')
    end
